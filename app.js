...
function addUrlRow(entry={label:"",url:""}){const row=document.createElement("div"),hasUrl=!!entry.url;row.className=`url-row${hasUrl?"":" editing"}`;row.innerHTML=`<input type="text" value="${esc(entry.label||"")}" aria-label="Link name" placeholder="Link name"><span class="url-value"><a href="${esc(entry.url||"#")}" target="_blank" rel="noopener noreferrer">${esc(entry.url||"")}</a><input type="url" value="${esc(entry.url||"")}" aria-label="URL" placeholder="https://example.com"></span><button type="button" class="url-edit" aria-label="${hasUrl?"Edit":"Finish editing"} URL" title="${hasUrl?"Edit URL":"Finish editing"}">${hasUrl?"✎":"✓"}</button><button type="button" class="url-remove" aria-label="Remove URL">×</button>`;const input=row.querySelector('input[type="url"]'),link=row.querySelector(".url-value a"),edit=row.querySelector(".url-edit");const toggleEdit=()=>{if(row.classList.contains("editing")){const value=input.value.trim();if(value){link.href=normalizeNodeUrl(value);link.textContent=value;row.classList.remove("editing");edit.textContent="✎";edit.title="Edit URL";edit.setAttribute("aria-label","Edit URL");}}else{row.classList.add("editing");edit.textContent="✓";edit.title="Finish editing";edit.setAttribute("aria-label","Finish editing URL");input.focus();input.select();}};edit.onclick=toggleEdit;input.onkeydown=event=>{if(event.key==="Enter"){event.preventDefault();toggleEdit();}};row.querySelector(".url-remove").onclick=()=>row.remove();$("urlRows").append(row);if(!hasUrl)setTimeout(()=>input.focus(),0);}
